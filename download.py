...
class Downloader:
    ...
    def setup(self):
        if os.path.exists(self.setup_folder):
            tqdm.write("data set cs276 is already setup")
        elif os.path.exists(self.data_zip_name):
            if self.__unzip_data():
                tqdm.write("the data setup is done")
        else:
    ...
    def __download_data_zip(self):
        try:
            ...
            tqdm.write(f"Could not download the data set: {e}")
            return False

    def __unzip_data(self):
        try:
            if not(os.path.exists(self.save_folder + "pa1-data")):
    ...
    def __setup_nltk(self):
        ...
        try:
            lem = nltk.stem.WordNetLemmatizer().lemmatize("schools")
        except LookupError:
